...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    input.slice(from,until).zipWithIndex.foldLeft(0f)(
        (curMax, e) => max(e._1/(from +e._2),curMax)
      )
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def downsweepSequentialAux(i: Int, end: Int, angle:Float): Unit = {
      if(i < end){
        output(i) =  max(input(i)/i , angle)
        downsweepSequentialAux(i+1, end, output(i))
      }
    }

    downsweepSequentialAux(from, until, startingAngle)
  }
