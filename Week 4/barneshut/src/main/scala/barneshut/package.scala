...
package object barneshut {
  ...
  extends Quad {
    val mass: Float = bodies.aggregate(0f)((acc, body) => acc + body.mass, _+_)
    val massX: Float = bodies.aggregate(0f)((acc, body) => acc + body.x * body.mass, _+_) / mass
    val massY: Float = bodies.aggregate(0f)((acc, body) => acc + body.y * body.mass, _+_) / mass
    ...
    def insert(b: Body): Quad = {
      ...
      if (size <= minimumSize)
        ...
        new_bodies.foldLeft(Fork(n_nw, n_ne, n_sw, n_se))((a, b) => a.insert(b))
      }
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      val x: Int = ((min(max(boundaries.minX, b.x), boundaries.maxX) - boundaries.minX) / sectorSize).toInt
      val y: Int = ((min(max(boundaries.minY, b.y), boundaries.maxY) - boundaries.minY) / sectorSize).toInt
      this.apply(x, y) += b
      this
    }
