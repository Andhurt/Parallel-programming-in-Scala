...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    means.map(x=> (x , map.getOrElse(x , GenSeq.empty))).toMap
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    if (!converged(eta)(means,updateMeans)) kMeans(points, updateMeans, eta) else updateMeans // your implementation need to be tail recursive
  }
}
